...
X_train, X_test, y_gravidade_train, y_gravidade_test = train_test_split(
    X, y_gravidade, test_size=0.2, random_state=42)

modelo_gravidade = RandomForestRegressor(n_estimators=300, random_state=42)
...
feature_importance = pd.DataFrame({
    'Feature': X.columns,
    'Importância': modelo_gravidade.feature_importances_
}).sort_values('Importância', ascending=False)


print("\n----- MODELO DE CLASSIFICAÇÃO PARA CLASSE -----")
clf_direto = RandomForestClassifier(n_estimators=300, random_state=42)
...
clf_com_gravidade = RandomForestClassifier(n_estimators=300, random_state=42)
...
try:
    ...
    resultados.to_csv('/Users/larasantos/Downloads/resultados_previsoes.csv', index=False)
    print("\nResultados completos salvos em 'resultados_previsoes.csv'")
